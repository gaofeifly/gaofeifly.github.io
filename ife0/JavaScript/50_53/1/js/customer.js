...
Customer.prototype.order = function(dishes,customer){
  ...
  waiter.doAJob(dishes,customer);
  unOrderNum--;
  customerStates[customer.seatPos].innerHTML = "等待上菜";
}
Customer.prototype.eat = function(dish,customer){
  // 认为有没吃完的菜时又上来新菜则两个菜同时吃
  updateCusDishOk(dish,customer);
...
function comeCustomers(customers){
  ...
  for(var i=0;i<customers.length;i++){
    ...
    customers[i].ordered = false;     // true表示已经点菜了
...
// 更新顾客菜品食用状态
function updateCusDishOk(dishName,customer){
  var div = null;
